...
async function main() {
  const search = await client.search.create({
    query: "climate change research",
    searchDomainFilter: ["science.org", "pnas.org", "cell.com"],
    maxResults: 10,
  });
...
<CardGroup cols={2}>
  ...
  <Card title="Date & Time Filters" icon="calendar" href="/guides/search-date-time-filters">
    Filter search results by recency and date ranges
  </Card>

  <Card title="Domain Filtering Guide" icon="globe" href="/guides/search-domain-filter-guide">
