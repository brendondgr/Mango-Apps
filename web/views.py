...
# Flask app for Calendar
from apps.Calendar.app import app as calendar_flask_app

# Flask app for Jobs
from apps.Jobs.app import application as jobs_flask_app
...
@csrf_exempt
def flask_proxy_calendar(request, path=''):
    """Proxy requests to the Flask Calendar app"""
    from werkzeug.serving import WSGIRequestHandler
    from werkzeug.wrappers import Request as WerkzeugRequest, Response as WerkzeugResponse
    from io import BytesIO
    
    flask_app = calendar_flask_app
    
    # Build environ dict for Flask
    environ = {
        'REQUEST_METHOD': request.method,
        'SCRIPT_NAME': '/pr/calendar',
        'PATH_INFO': '/' + path if path else '/',
        'QUERY_STRING': request.META.get('QUERY_STRING', ''),
        'CONTENT_TYPE': request.META.get('CONTENT_TYPE', ''),
        'CONTENT_LENGTH': request.META.get('CONTENT_LENGTH', ''),
...
@csrf_exempt
def flask_proxy_jobs(request, path=''):
    """Proxy requests to the Flask Jobs app"""
    from werkzeug.serving import WSGIRequestHandler
    from werkzeug.wrappers import Request as WerkzeugRequest, Response as WerkzeugResponse
    from io import BytesIO
    
    flask_app = jobs_flask_app
...
def pr_view(request):
    ...
    apps_list = [
        ...
        {
            'name': 'Jobs Finder',
            'description': 'Search and track job applications.',
            'icon': 'briefcase',
            'url': '/pr/jobs/',
            'active': False
        },
